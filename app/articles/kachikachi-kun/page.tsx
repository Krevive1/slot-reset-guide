...
const title = "カチカチくん（小役カウンター）とは？使う場面と選び方";
const description =
  ...
  return (
    <article>
      ...
      <div className="article">
        ...
        <p>
          スマスロモンキーターンVでは、通常時に成立する5枚役の出現率が設定推測の参考情報になるとされています。
          解析情報では、通常時・AT中を問わず同じ確率で成立するとされ、カウント対象に含めて差し支えないとされています。
        </p>
        <p>
          朝イチから打つ場合は、最初から数えておく価値があります。途中から数え始めても、それ以前に成立した5枚役の回数は復元できません。
          朝イチからカウントしておけば、実戦を続けるかどうかを判断するときの材料を一つ増やせます。
        </p>
        <p>
          ただし、短時間では出現率が大きく偏る可能性があり、5枚役の回数だけで設定を断定することはできません。
          終了画面、初当たり、直撃、その他の設定示唆などとあわせて総合的に判断してください。
          また、5枚役は設定推測の要素であり、リセット（設定変更）そのものを判別する要素とは別のものです。
        </p>
        <p>
